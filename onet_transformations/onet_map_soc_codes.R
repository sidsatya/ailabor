...
# Define list of years for which to read data
years <- 2003:2025
...
for (year in years) {
  # Construct the variable name for the file path (e.g., onet_2003_release)
  file_path_var_name <- paste0("onet_", year, "_release")
  
  # Check if the variable exists and get its value
  if (exists(file_path_var_name, envir = .GlobalEnv)) {
    file_path_to_read <- get(file_path_var_name, envir = .GlobalEnv)
    
    if (file.exists(file_path_to_read)) {
      data_table_name <- paste0("onet_", year, "_data")
      onet_data_list[[data_table_name]] <- fread(file_path_to_read, stringsAsFactors = FALSE)
      message("Read: ", file_path_to_read)
    } else {
      warning(paste("File defined in", file_path_var_name, "does not exist:", file_path_to_read))
    }
  } else {
    warning(paste("File path variable", file_path_var_name, "not defined for year", year))
  }
}

# Loop to convert SOC codes to 2010 SOC codes and print NA rows
for (year in years) {
  data_table_name <- paste0("onet_", year, "_data")
  
  if (exists(data_table_name, where = onet_data_list)) {
    current_dt <- onet_data_list[[data_table_name]]
    
    # add ONET_release_year column
    current_dt[, ONET_release_year := year]

    # Ensure the O*NET-SOC Code column exists
    if ("O*NET-SOC Code" %in% names(current_dt)) {
      current_dt[, `O*NET 2010 SOC Code` := sapply(`O*NET-SOC Code`, convert_to_2010_soc, year = year)]
      current_dt[, `O*NET 2018 SOC Code` := sapply(`O*NET-SOC Code`, convert_to_2018_soc, year = year)]
      
      message(paste("\nNA rows in", data_table_name, "for O*NET 2010 SOC Code:"))
      na_rows <- current_dt[is.na(get("O*NET 2010 SOC Code"))]
      
      if (nrow(na_rows) > 0) {
        print(na_rows)
      } else {
        message("No NA rows found for 2010 SOC Code.")
      }

      message(paste("\nNA rows in", data_table_name, "for O*NET 2018 SOC Code:"))
      na_rows <- current_dt[is.na(get("O*NET 2018 SOC Code"))]
      
      if (nrow(na_rows) > 0) {
        print(na_rows)
      } else {
        message("No NA rows found for 2018 SOC Code.")
      }
      # Update the list with the modified data.table
      onet_data_list[[data_table_name]] <- current_dt
    } else {
      warning(paste("Column 'O*NET-SOC Code' not found in", data_table_name))
    }
  } else {
    warning(paste("Data table", data_table_name, "not found in onet_data_list."))
  }
}


columns_of_interest <- c("O*NET-SOC Code", "O*NET 2010 SOC Code", "O*NET 2018 SOC Code", "ONET_release_year",
                         "Task ID", "Task", "Task Type", "Incumbents Responding", "Date", "Domain Source")

# Combine all data.tables in the list
# use.names = TRUE ensures matching by column name
# fill = TRUE adds missing columns (like "Date") and fills with NA
all_onet_data_harmonized <- rbindlist(onet_data_list, use.names = TRUE, fill = TRUE)

# After combining, you might want to select only the columns of interest.
# It's good practice to ensure you only try to select columns that actually exist
# in the combined data.table, especially if columns_of_interest might contain
# names not present in any of the source files.
final_selected_columns <- intersect(columns_of_interest, names(all_onet_data_harmonized))

# Create the final data.table with only the selected columns
# The .. prefix tells data.table to look for the 'final_selected_columns' variable 
# in the calling environment.
all_onet_data_final <- all_onet_data_harmonized[, ..final_selected_columns]

# Group by 'Task' and count occurrences
task_counts <- all_onet_data_final[, .N, by = "Task"]

# Rename the count column for clarity (optional)
setnames(task_counts, "N", "Count")

print(task_counts[order(-Count)])

# save task counts to a CSV file
output_file <- file.path(ROOT_DIR, "onet_transformations/intermediate_data/unique_task_statements.csv")
fwrite(task_counts, output_file, bom=FALSE, encoding = "UTF-8")

# Replace NULL values with NA in all columns
# This is needed because fwrite can't handle NULL values in list columns
all_onet_data_final <- as.data.table(lapply(all_onet_data_final, function(x) {
  if(is.list(x)) {
...
# save all ONET data to a CSV file
output_file_all_onet <- file.path(ROOT_DIR, "onet_transformations/intermediate_data/all_onet_data_mapped_soc_codes.csv")
fwrite(all_onet_data_final, output_file_all_onet, bom=FALSE, encoding = "UTF-8")
